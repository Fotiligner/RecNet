...
class LanguageLossTrainer(Trainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            # print(interaction['item_id_list'])
            # print(interaction['item_length'])
            # # continue
            # exit()

            loop = asyncio.get_event_loop()
            loop.run_until_complete(loss_func(interaction))
        return total_loss
...
class ITEMLanguageLossTrainer(LanguageLossTrainer):
    ...
    def evaluate(
        ...
        eval_func = self._full_sort_batch_eval # 确定evaluate的函数
        if load_best_model:
            checkpoint_file = model_file or self.saved_model_file
            checkpoint = torch.load(checkpoint_file, map_location=self.device)
            self.model.load_state_dict(checkpoint["state_dict"])
